
<div>
<div style="position:absolute;">
    <b style="display:left; font-size:30px;">Schelves Version 1.0</b> 
</div>
<div style="position:relative; margin-bottom :10px; margin-left:1000px;">
    
<form method="POST" action="index.php?q=repository&categ=search&sub=default">
        <input type="texte" name="words" style="width:300px;height:25px;border:solid 2px grey; border-radius:12px" />
        <input type="submit" name="Rechercher" style="width:150px;height:30px;border:solid 2px grey; border-radius:12px" />
        <a href="#">avancée</a>
    </form>


</div>
</div>
...
class user extends connexion{
    ...
    public function connect($user_pseudo, $user_user_password) {
        $stmt = $this->getCnx()->prepare('SELECT * FROM user WHERE user_pseudo = :user_pseudo AND user_user_password = :user_user_password');
        $stmt->execute([':user_pseudo' => $user_pseudo, ':user_user_password' => $user_user_password]);
        if ($stmt->rowCount() > 0) {
    ...
    public function deleteUser($user_pseudo, $user_user_password) {
        $stmt = $this->getCnx()->prepare('SELECT * FROM user WHERE user_pseudo = :user_pseudo AND user_user_password = :user_user_password');
        $stmt->execute([':user_pseudo' => $user_pseudo, ':user_user_password' => $user_user_password]);
        if ($stmt->rowCount() > 0) {
